...
describe('financial ticker', () => {
  it('should render the right information', () => {
    ...
    expect(tickerItem.querySelector('.tickerSymbol').innerHTML).toEqual('S&amp;P 500');
    expect(tickerItem.querySelector('.lastPrice').innerHTML).toEqual('3372.2');
    expect(tickerItem.querySelector('.tickerChangePercentage').innerHTML).toEqual('0.38%');
    expect(tickerItem.querySelector('.tickerCurrentPrice').innerHTML).toEqual('12.9');
    expect(tickerItem.querySelector('.tickerMovement').getAttribute('alt')).toEqual('up');
  ...
  it('should render change={false} negative', () => {
    ...
    render(
      <ReactTicker>
        <FinancialTicker id={1} change={false} symbol="S&P 500" lastPrice="3372.2" percentage="0.38%" currentPrice="12.9" />
      </ReactTicker>
      , div)

    console.log(div.innerHTML)
    const tickerItem = div.querySelector('.tickerItem');
    expect(tickerItem.querySelector('.tickerMovement').getAttribute('alt')).toEqual('down');
